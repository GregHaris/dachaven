import { getOrdersByProductListings } from '@/lib/actions/order.actions';
...
const Orders = async ({ searchParams }: SearchParamProps) => {
  ...
  const productListingId =
    (resolvedSearchParams.productListingId as string) || '';
  const searchText = (resolvedSearchParams.query as string) || '';

  const orders = await getOrdersByProductListings({
    productListingId,
                  ...
                  orders.map((row: IOrderItem) => (
                    ...
                    >
                      ...
                      <td className="min-w-[200px] flex-1 py-4 pr-4">
                        {row.productListingTitle}
                      </td>
                      <td className="min-w-[150px] py-4">{row.buyer}</td>
                      <td className="min-w-[200px] py-4">{row.buyerEmail}</td>
